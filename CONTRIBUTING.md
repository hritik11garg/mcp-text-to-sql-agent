...
**And the browser client, which is a separate toolchain and not a lower standard** — CI runs it as its own job, so skipping it locally only moves the failure:

```powershell
cd web
npm run typecheck      # tsc --noEmit, strict, with noUncheckedIndexedAccess
npm test               # vitest; 15 of these are generated by fast-check
```
...
2. Its tests pass in CI. **The pipeline landed 2026-08-08** — `.github/workflows/ci.yml`, four jobs on every push and pull request: **lint** (`ruff check`, `ruff format --check`, `mypy --strict`), **python** (the whole suite, then the security layer again by name), **web** (`tsc`, `vitest`, a production build), **docs** (relative links and anchors). A missing Docker daemon *fails* the run rather than skipping, so the security gate cannot report green over tests it never executed — [TESTING.md](docs/development/TESTING.md) §14. **Branch protection is not on yet**, so CI reports rather than blocks; that is a repository setting and it is the half that makes a green run mean something.
