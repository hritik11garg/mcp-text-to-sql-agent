...
class TestReadiness:
    async def test_all_probes_up_is_ready(self) -> None:
    ...
    async def test_the_failure_reason_never_reaches_the_status(self) -> None:
        """A driver error carries the DSN. Only the verdict may be published."""
        _, detail = await Readiness([Probe("database", failing)]).status()
...
class TestReadinessCaching:
    async def test_a_repeated_call_does_not_rerun_the_probe(self) -> None:
...
class TestProbesAreIsolated:
    ...
    async def test_any_exception_type_is_contained(self, exc: type[BaseException]) -> None:
        ...
        if issubclass(exc, Exception):
            assert await readiness.status() == (False, {"database": DOWN})
        else:
