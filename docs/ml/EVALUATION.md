...
**Resumability is not a convenience here.** Free-tier models cap tokens per model per day, so a 200-question run spans most of a budget and being stopped at question 140 is an ordinary operating condition. Every question is written as it completes and a re-run skips what is already **answered**.

Answered, not recorded — and until 2026-08-06 it was recorded, which meant the design did not actually do the thing it was designed for. The first full-split attempt hit the daily cap at question ~395 of 1034 and recorded 308 questions as `llm_failed`. Every one was thereafter permanently done, so the run could only be restarted from nothing, and the next day's budget would have stopped in the same place. A question whose `error_type` says the system under test was never asked is now re-attempted, overwriting its record ([ADR-037](../architecture/DECISIONS.md#adr-037--resumption-skips-answered-questions-not-recorded-ones)). This is the same predicate that excludes those questions from the scored denominator in §5, and a test asserts the two agree.

**A run also stops at a wall.** Ten consecutive infrastructure failures end it (`--halt-after`). A spent budget does not recover mid-run, so continuing means asking a dead provider once per remaining question — wall clock spent burying the cause under identical records. The threshold is low enough to trip on a whole database failing as `scope_unavailable`, which is a deployment fault worth stopping for rather than scoring around.

The other corollary is a refusal: a resumed run must have the **same configuration fingerprint** — dataset, split, model, retriever version, prompt version, commit and seed. Resuming after any of those change would produce a results directory that is half one configuration and half another, and every number computed from it would be a weighted average of two things nobody meant to average. The commit is in the fingerprint deliberately: fixing a bug and re-running is the easiest way to do this by accident.
...
| Baseline retriever (`retrieval-only`, `k=30`) | **80.9%** | **0.936** | **3.3%** | — | TBD — Stage 6 | TBD |
...
**The single measured row is not a benchmark result and the table must not be read as one.** It covers 379 of 921 scoreable questions — 9 of 20 databases — from a full-split run paused on a daily token cap and resuming into the same directory ([BENCHMARKS.md](BENCHMARKS.md) §1.1). `k` belongs to the row: the same configuration at `k=10` scores 42.7%.
