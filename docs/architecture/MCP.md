...
> **Status: implemented, and one server is now measured over the wire.** All four servers ship, and the schemas below are the ones they publish — asserted against a committed snapshot in `tests/contract/tool_schemas.json`, so this page and the code cannot drift. The client half (`tools/list` discovery) ships too.
>
> **The question this block used to leave open — do the servers answer identically to the components they wrap? — is answered for `schema_search`.** Measured 2026-08-10: all 1,034 Spider dev questions asked both ways, ordered element lists identical **1,034 times out of 1,034**, at a constant **+7.8 ms** per call ([BENCHMARKS §8](../ml/BENCHMARKS.md), §7.1 below for the scoping this cost). The eval harness gained an `mcp-retrieval` baseline that substitutes the MCP path at the composition root, which is exactly the run this paragraph used to describe as the way to close it.
>
> **Still open:** Streamable HTTP transport, and the same proof for the other three servers. `validate_sql`, `execute_sql` and `profile_table` are proven by the contract suite — schemas and error shapes — and by no benchmark. The published **accuracy** figures in [BENCHMARKS.md](../ml/BENCHMARKS.md) still measure the direct answering path; what is now measured is that retrieval over stdio feeds that path the identical context.
>
