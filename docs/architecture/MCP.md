...
> **Status: implemented.** All four servers ship, and the schemas below are the ones they publish — asserted against a committed snapshot in `tests/contract/tool_schemas.json`, so this page and the code cannot drift. The client half (`tools/list` discovery) ships too. Still open: Streamable HTTP transport, and **proving the servers answer identically to the components they wrap** — which no longer waits on the harness, because the harness exists and has produced a baseline. It waits on a decision this page should state plainly: **the eval drives the components directly, not the servers.** So every accuracy number in [BENCHMARKS.md](../ml/BENCHMARKS.md) measures the code the servers call, and none of them measures the servers. The contract tests prove the schemas and the error shapes; they do not prove that a question answered through stdio scores the same as one answered in-process. Closing that means a run with the MCP path substituted at the composition root, against the same `--run-id` configuration.
>
