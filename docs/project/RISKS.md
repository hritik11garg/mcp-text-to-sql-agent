...
**Mitigation.** Negative tests that must fail, gating the first slice rather than a hardening pass at the end. The commonly-missed case is function execution — `pg_read_file` and `COPY ... TO PROGRAM` are reachable from a `SELECT`-only role unless `EXECUTE` is revoked. Explicitly tested.
...
**Mitigation — implemented.** Split by database. A `db_id` hashes into exactly one band, a question naming a database with no assignment raises rather than defaulting into dev, splits are committed as a file rather than regenerated, and membership depends only on the database's own name, so growing the corpus cannot move a held-out database into train ([ADR-021](../architecture/DECISIONS.md#adr-021--splits-are-a-hash-of-the-database-name-not-a-seeded-shuffle)).

**This paragraph read *"disjointness holds by construction"* until 2026-08-12, and the risk had materialised on 2026-08-11.** The claim was true about the wrong property. The bands are disjoint *within themselves*, which is what the hash guarantees — but the reported evaluation set is **Spider's own `dev.json`**, defined outside the hash and unknown to it. So the split was internally consistent while **11 of those 20 databases sat in `train`** carrying 605 questions, and five more in `dev`/`smoke` ([ADR-047](../architecture/DECISIONS.md#adr-047--spiders-own-split-is-the-evaluation-boundary-and-the-training-set-is-carved-around-it)). **Splitting by database prevents question-level leakage and says nothing about an evaluation set the splitter has never heard of.**

**Closed by reservation.** `assign(reserved=...)` forces all twenty to held-out regardless of hash, and `leaked_databases()` is an executable check that `tests/unit/test_split_disjointness.py` runs against the committed assignment — Stage 5's disjointness task, landed before Stage 5. See the [register below](#materialized-risks) for why this entry was stale for a day.
...
**High × Medium.** 30 documents written before the code is a lot of surface to keep honest.
...
**The honest read:** this risk is real and partly accepted, and it is **the most frequently materialised risk on this page** — **seven occurrences**, counted as rows in the table below. A document describing intent that the code later contradicts is worse than no document. The `TBD` markers and stage gates keep it manageable; only assertions keep it honest, and most of this surface cannot be asserted.

> **The 2026-08-12 row is the worst-shaped instance so far, and it is worth reading before the others.** Every prior occurrence was a *statement* that had gone false — findable by re-reading the sentence against the code. That one was a **comment describing a check**, sitting in the CI file, next to the step it claimed to describe. Drift of that shape does not merely mislead a reader; it **ends their audit**, because the thing they came to verify appears already verified. The two responses that scale are the two taken here: turn the claim into an executable assertion, or delete it. Leaving it as better-worded prose fixes nothing.

> **Recounted 2026-08-11, and the recount is itself an instance.** This paragraph read *"six occurrences"* and [ENGINEERING_MATRIX](ENGINEERING_MATRIX.md) §40 read *"seven"*, and **neither was reproducible from any count in the repository** — the table held three R-17 rows, one of which described three contradictions at once, so every tally depended on whether a reader counted rows or incidents. The count is now **rows in the register**, which is the only rule two documents can apply and get the same answer. Two more rows were owed and missing: the 2026-08-10 reconcile commits fixed drift and recorded none of it, which is the exact defect the note under the table describes.

**The pattern is worth naming: the drift is almost never in the document that was edited.** A slice updates the page it is about, and the stale claim turns up somewhere adjacent — a status block at the top of the same file, a *planned* table naming something that shipped under another name, a demo script narrating a number a later measurement overturned. That is why the audits are periodic and whole-tree rather than per-slice.

**And the register is not exempt from the risk it registers.** It has now gone stale twice in the same way: once saying *"None yet"* while the entry above it said three, and once missing two occurrences because the commits that fixed the drift did not record having found it. **A risk register updated only by the slices that remember it undercounts in exactly the direction that flatters the project.**
...
| 2026-08-08 | **R-14** · LLM spend runs away during eval | The cap halted the full-split run a **third** time. Three days, three budgets, 921 questions | **Working as designed, and now with a completed run to show for it.** Days 1 and 2 each ended with 12 quota failures; day 3 re-attempted rather than retired them, so the final result carries **zero** infrastructure errors |
| 2026-08-08 | **R-01** · fine-tune shows no measurable gain | Predicted, and the full split narrows the window further: R@20 finished at **0.9973**, leaving 0.27 points of headroom at rank 20 and 5.65 at R@5 | Not resolved — **sharpened**. Ablation A2 is near-unwinnable on Spider by construction, which is now stated in the ablation table rather than discovered during Stage 5 |
| 2026-08-08 | **R-17** · documentation drifts | Building the UI exposed an asymmetry no document described: the stream announced `validate` on the `explain_only` path but nothing for execution on the ordinary one, so a client had to infer a phase from a data event | Fixed in the code rather than documented as a quirk. **Found by the first consumer that had to render progress** — a category of drift no audit of the documents would have caught, because every document agreed with every other |
| 2026-08-10 | **R-17** · documentation drifts | Two reconcile commits found and fixed drift across the tree — including OBSERVABILITY claiming span boundaries were wired when nothing imports `opentelemetry`, and a priorities list that had gone stale about itself. **Neither commit recorded an occurrence here**, so this row was written retroactively during the next audit | Fixed in the documents. The lesson is about the register, not the drift: **a commit that fixes drift and does not log it makes the risk look rarer than it is** |
| 2026-08-11 | **R-12** · train/eval leakage inflates Recall@k | **The one risk on this page whose mitigation was marked *implemented* while the property it claimed did not hold.** Hashing Spider's twenty dev database names by [ADR-021](../architecture/DECISIONS.md#adr-021--splits-are-a-hash-of-the-database-name-not-a-seeded-shuffle)'s rule put **11 in `train`, carrying 605 questions**, and five more in `dev`/`smoke` — **16 of the 20 databases every published number is measured on**, in bands this project would have fitted to or tuned against. Found by an audit of the split question, not by a failure: it raises no error and produces no implausible number, only a **better** Recall@k, arriving exactly when a fine-tune is being judged on Recall@k | **Caught before it could affect anything** — no fine-tune has run, so no published figure is contaminated. Fixed by reservation rather than a new policy: `assign(reserved=...)` forces all twenty to held-out regardless of hash, and `leaked_databases()` + `tests/unit/test_split_disjointness.py` check the committed assignment ([ADR-047](../architecture/DECISIONS.md#adr-047--spiders-own-split-is-the-evaluation-boundary-and-the-training-set-is-carved-around-it)). **The lesson is the same one the R-17 rows keep teaching**: `e2db933` updated DATASETS, TASKS, DECISIONS and CHANGELOG and did not touch this register, TRAINING.md's disjointness rule, or TROUBLESHOOTING's *"Recall@k is suspiciously high"* entry — the three documents a reader would consult *about this exact defect* |
| 2026-08-11 | **R-17** · documentation drifts | The largest occurrence yet, and the one this risk predicts most exactly. After the MCP baseline landed, **seven** places still asserted the servers were "proven to answer as well by nothing" — README twice, MCP.md's own status block above the new §7.1, TESTING, DEMO_SCRIPT and the matrix — while three other places asserted the measured opposite. Alongside it: EVALUATION §6's header contradicting its own next paragraph, six drifted counts, a benchmark row citing a commit where its script does not exist, and a CHANGELOG nine commits behind | Fixed in this commit. **Not one of these was reachable by the link checker** — every link resolved and every anchor was valid; the false statements were prose. That bounds what a mechanical check can do here and is the argument for the periodic whole-tree audit rather than a replacement for it |
| 2026-08-12 | **R-17** · documentation drifts | **CI had never once passed, across all four runs since the workflow landed on 2026-08-08**, and every document describing the pipeline was written as though it had. The Python job failed at the same step every time: nine modules import shared helpers as `from tests.conftest import ...`, which resolves under `python -m pytest` (the working directory goes on `sys.path`) and **fails to collect** under the `pytest` console script (it does not). Local runs used one form, CI used the other, so *"1,634 tests pass"* and *"CI is green"* were true and false about two different commands | `pythonpath = ["."]` in `pyproject.toml`, so both invocations are the same command; [TESTING §11](../development/TESTING.md) now states the rule in one line. **The lesson is not the `sys.path` detail** — it is that a red pipeline sat unread for four commits. A check nobody consumes is indistinguishable from a check that does not run, which is [the unmarked-test defect](../development/TESTING.md) exactly one layer out |
| 2026-08-12 | **R-17** · documentation drifts | **A comment claiming a check that did not exist**, which is the sharpest form this risk takes. CI's build step said it *"proves `assetsInlineLimit: 0` still holds"* — the Vite option the served Content-Security-Policy depends on — and `vite build` succeeds either way, so an auditor reading that line would have concluded the property was guarded ([SECURITY.md §13.18](../operations/SECURITY.md)). Alongside it, four claims that had gone stale in the *flattering* direction and one in the other: DEPLOYMENT.md headed **"TBD — v2.0"** over a Dockerfile that had been built and run; ROADMAP's Stage 6 row reading **0% / Not started** while nine of its eighteen items were done; Stage 2 reading 82% against a real 91%; and `fail_under = 85` sitting in `pyproject.toml` since Stage 0 with **nothing ever running `--cov`** | Fixed in this commit, and two of them **structurally rather than textually**: the CSP claim is now `assert-no-inline-assets.mjs` (verified red on a planted violation before being trusted), and the coverage floor now runs on the suite CI already executes. The other three were recounts. **The comment case is the one to remember** — prose that describes a check reads exactly like a check, and it is the only kind of drift that makes an auditor *stop looking* |
