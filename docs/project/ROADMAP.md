...
**What is genuinely blocking, in order:** finishing the full-split run — it is under way and reached 9 of 20 databases before a daily token cap paused it, which is where the free tier puts the ceiling rather than anything in the code. Then SSE and the demo UI it feeds (blocks the Stage 1 close-out and any *visual* demo — `POST /v1/query` now answers a question, but a curl is not a screenshot), then the agent loop.
...
- [~] **A browser can ask a question and watch it being answered.** `POST /v1/query` answers one — verified against a real Spider schema, question in, SQL and rows out. What is missing is the *watching*: SSE and the React UI over it. Added after noticing the project had no surface a reader could see — the MCP servers are a capability, not a demo, and until this exists there is nothing to put in a README GIF either
