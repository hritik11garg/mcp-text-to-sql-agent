...
Layers overlap by design — a security test needing a real database carries both markers — so the rows sum to more than the total. The `property` row counts Python only; **`web/` holds 15 more, generated by `fast-check`**, and no single command counts both languages. 14 source packages · 127 web tests · 26 documents · `docker-compose.yml` present, **CI landed 2026-08-08** (§24), **no Dockerfile** (§22).
...
**Applicable?** Yes — 171 npm packages and a Python dependency set sit under a page that renders untrusted output.
