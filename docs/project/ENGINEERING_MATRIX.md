...
| 🟡 Partial | **21** | Gaps named per row |
| 🔴 Applicable and absent | **2** | §14, §15 — both authentication, and both v2.0 |
...
Layers overlap by design — a security test needing a real database carries both markers — so the rows sum to more than the total. The `property` row counts Python only; **`web/` holds 15 more, generated by `fast-check`**, and no single command counts both languages. 15 source packages · 127 web tests · 26 documents · **Dockerfile and a four-service Compose stack, run from empty volumes 2026-08-11** (§22) · **CI landed 2026-08-08 and first passed 2026-08-12** (§24).
...
**Proof.** 127 tests (`web/`), `npm run typecheck`, `docs/development/TESTING.md` §13.
...
**Gaps — all operational, all v2.0.** No vacuum/bloat monitoring, no WAL policy, no backup or restore procedure. Not applicable while the only data is a reproducible benchmark import.
...
**That failure mode is now ruled out for retrieval, by measurement.** 2026-08-10: every one of the **1,034** Spider dev questions was asked twice — `SchemaRetriever.search` in process, and `search_schema` on a subprocess over stdio — and the ordered element lists matched **1,034 times out of 1,034**, at a constant **+7.8 ms** per call. `scripts/compare_mcp_retrieval.py`, [BENCHMARKS](../ml/BENCHMARKS.md) §8, [ADR-045](../architecture/DECISIONS.md).

**Gap — narrowed, not closed, and 🟡 rather than 🟢 for a stated reason.** Only `search_schema` has been measured. `validate_sql`, `execute_sql` and `profile_table` cross contract tests and no benchmark. A `with-validation`-over-MCP baseline would move two hops at once and could not attribute a difference to either, so it is a separate measurement that does not exist yet.
...
**Implemented?** **Yes, as of 2026-08-11.** A five-stage `Dockerfile` — a Node stage for the bundle, a pip stage, a slim runtime, and one-shot `migrate` / `api` targets. Runs as `appuser` (uid 10001). Bases pinned by tag; every Python dependency pinned by `==`. `torch` installed from PyTorch's CPU index *before* the rest, so the resolver never pulls a CUDA build — over a gigabyte not downloaded. Model weights are a named volume rather than a layer, created in the image with the right ownership because a Docker volume inherits ownership from the image path at first mount.

**Proof.** `Dockerfile`, `.dockerignore`, `docker-compose.yml`; run end to end from **empty volumes** on 2026-08-11 — build, migrate, seed, serve, and a real question answered in 2.7 s. [DEPLOYMENT.md](../operations/DEPLOYMENT.md) §1–2.

**No secrets in layers, and it is the `.dockerignore` that holds that**, not discipline: `.env*` is excluded with an explicit `!.env.example` re-inclusion, alongside `.git/`, `data/`, `results/` and `web/node_modules/`. Configuration reaches the container through `env_file` at run time.

**Failure mode.** A container built without these constraints — root user, writable filesystem, secrets baked into layers — is the default outcome of building one in a hurry. The note this row used to carry (*"correctly not built yet: an unused Dockerfile rots"*) was right for as long as it was true, and stopped being true when a reader could not run the project without one.

**Gaps — why this is 🟡 and not 🟢.** No image scanning in CI (§25 covers dependencies, not layers). **No read-only root filesystem** and no dropped capabilities; both are compose-level settings that were not verified against the model cache's write path, and claiming them untested would be worse than naming them. No image signing or provenance attestation.
...
**Applicable?** Yes — **157 npm packages, of which 3 reach production** (`react`, `react-dom` and one transitive), plus a Python dependency set, sit under a page that renders untrusted output. The gap between 157 and 3 is the point: almost the whole npm surface is build and test tooling, which is why §25's audits report the two trees separately.
...
**Gaps.** No metrics endpoint, no OpenTelemetry traces (documented intent, unbuilt), no alerting. v2.0.
...
**Gaps — stated on the row itself.** That measurement is of a **free tier**, not of this system: the dominant term is a shared provider queue. Component budgets (retrieval < 100 ms, validation < 50 ms) remain unmeasured. It does not close the saturation work, which is v2.0.
...
**Still absent.** Throughput, latency under sustained load, memory growth over hours, and behaviour past pool saturation against a real database. Those need a running server, a load generator, and somewhere to run it — v2.0. The distinction this row now draws: **concurrency is a correctness property and belongs on every commit; load is a measurement and belongs in a pipeline.**
...
**Implemented?** **All three targets**, all shallow, and none of them a campaign.
...
**The SSE parser**, added 2026-08-10 — the target this row named as untouched for two days. `web/src/api/sse.property.test.ts` generates wires from field lines with all three terminators mixed, and arbitrary UTF-16 alongside them, then splits each one at generated points and again one character at a time. The claim is that **where the chunks fall cannot change what comes out**, checked against a reference parser written by a different algorithm.

**Proof.** `tests/security/test_property_write_containment.py`, `tests/security/test_property_request_body.py`, `web/src/api/sse.property.test.ts`, `web/src/sql/tokenize.property.test.ts`; 500 generated inputs per property per CI run, in both languages.
...
**Why this is 🟡 and not 🟢.** All three targets are 100–500 short inputs per run from general-purpose strategies, with **no corpus, no coverage guidance, no dedicated fuzzing tool, and no persistence of interesting inputs between runs.** That is a smoke test wearing a generator. What *is* now done is mutation testing of the tests themselves — seven deliberate breaks to the two parsers, all seven caught — which is evidence the generators can fail, not evidence the search is deep.
...
**Implemented?** **Four of the five** properties named below — three on 2026-08-09, the fourth on 2026-08-10. `hypothesis` 6.165.2 for the 40 Python tests carrying the `property` marker, inside the existing layers rather than a directory of their own so the security gate selects them without knowing they are generated; `fast-check` 4.9.0 for the 15 in `web/`. 100 examples locally, 500 in CI, in both languages and keyed on the same variable.

**Proof.** `tests/security/test_property_write_containment.py` (no generated write is accepted, in five nesting positions — *and* no generated `SELECT` is refused, which is the half that stops the check being deleted) · `tests/security/test_property_sse_framing.py` (no payload whatsoever produces two events) · `tests/unit/test_property_row_limit.py` (the truncation algebra against an independently written reference model) · `web/src/sql/tokenize.property.test.ts` (the round trip over arbitrary UTF-16 and over every prefix of a generated query) · `web/src/api/sse.property.test.ts` (chunk boundaries cannot change the frames, against a second reference model) · shared generators in `tests/strategies.py` · [TESTING.md](../development/TESTING.md) §15.
...
**What the fourth one cost, and what it bought.** `fast-check` adds **two packages**, which is the test the `locust` pin failed (§25). It closed two rows at once: the tokenizer round trip here, and the SSE parser in §37. It also came with a lesson about generators — the mutation that stopped stripping the single space after `data:` **survived the first draft with every property green**, because the generator wrote `data:value` and no server sends that. A fixed separator where a generated one belonged, and only mutating the code found it.

**The one property not implemented**, named rather than left implied:
...
> **Struck-through rows are done and are kept rather than deleted**, so the list reads as a record of what was worked and in what order. **Row 3 closed on 2026-08-10** once Docker came back and the Spider databases were loaded — the blocker was infrastructure, exactly as this note had said. **Row 10 — authentication — is the whole of what remains**, and it is the last of the three 🔴 rows.
...
| 7 | ~~**Property-based tests** for the five invariants in §38~~ — **four of five done 2026-08-10** | §38 | 55 tests over two languages. Found an unhandled `TokenError` on the first run — a truncated string literal crashed validation instead of being refused. Then found **two defects in its own assertions** on 2026-08-10 when a warmed corpus reached inputs it never had: a proxy (`status < 500`) that a valid body broke, and a substring check that matched the application's own placeholder ([TESTING](../development/TESTING.md) §15). The one property left needs a live database per example |
...
| 9 | ~~**Fuzz the parsers**~~ — **all three targets done 2026-08-10** | §37 | The validator (found the `TokenError` crash), the request body (found the reflected field name, SECURITY.md §13.16), and the SSE parser. Stays 🟡: three shallow generators are not a campaign, and the row says so |
